...
namespace UnityEngine.XR.MagicLeap
{
    ...
    {
        // joa -> Var's
        // -> We will need access to MySceneManager to get the current/active Scene 
        MySceneManager mySceneManager;
        private int curSceneNum;
        // -> We will need access to the modal/sub "Menu" GameObject to show/hide on Home Button tap... 
        GameObject curSceneMenu;
        private bool curSceneMenuState = false;
        ...
        #region Joa Methods
        // joa -> My OnButtonUp event - Track the Controller Buttons - see the Callback created in Start() 
        private void OnButtonUp(byte controller_id, MLInputControllerButton button) 
        {
            // joa -> Bring up Menu when home button is tapped i toggle logic depends on "false" set on Start()
            if (button == MLInputControllerButton.HomeTap)
            {
                print("HOME BABY - From Scene #" + curSceneNum );
                if( curSceneNum > 1 )       // WE are NOT in MainMenu scene
                {
                    SetSceneMenuState(!curSceneMenuState);
                }
            }
            // joa -> Do something on Bumper??
            else if( button == MLInputControllerButton.Bumper)
            {
                print("BUMPED BABY");
            }
        }
        private void SetSceneMenuState( bool state)
        {
            curSceneMenuState = state;
            if (curSceneMenuState) 
                curSceneMenu.SetActive(true);
            else
                curSceneMenu.SetActive(false);
        }
        ...
        {
            string gestureType = _controllers[0].TouchpadGesture.Type.ToString();
            string gestureState = _controllers[0].TouchpadGestureState.ToString();
            string gestureDirection = _controllers[0].TouchpadGesture.Direction.ToString();
        ...
        */
        ...
        /* joa -> Make an update function to get controller data...see that convulated path to 
        private void Update()
        {
            print( "Trigger Val = " + _controllers[0].TriggerValue);
        }
        */
        ...
        protected override void Start()
        {
            ...
            AddController(0);
            AddController(1);                               // joa -> leaving in a lot of baggage for now...
            ...
            // joa -> add Callback for Controller Button(s) processing
            MLInput.OnControllerButtonUp += OnButtonUp;     // set the Callback
            MLInput.GetController(MLInput.Hand.Left);       // get the left Controller for now

            // joa -> get the MySceneManager Singleton so we can access what Scene we are in...
            // Then we can use it when processing input...
            mySceneManager = FindObjectOfType<MySceneManager>();
            curSceneNum = mySceneManager.GetCurrentSceneNumber();
            // joa -> get menu Object and disable if we are not in MainMenu scene...
            if( curSceneNum > 1 )
            {
                curSceneMenu = GameObject.Find("SceneMenu");
                SetSceneMenuState(false);
            }
        ...
        protected override void OnDestroy()
        {
            ...
            // joa -> remove my Callback for Button(s)
